...
impl RTC {
    ...
    pub fn new(interrupt_evt: EventFd) -> RTC {
        RTC {
            ...
            tick_offset: dbs_utils::time::get_time_ns(dbs_utils::time::ClockType::Real) as i64,
            match_value: 0,
            load: 0,
            imsc: 0,
            ris: 0,
    ...
    fn get_time(&self) -> u32 {
        ...
        (ts / dbs_utils::time::NANOS_PER_SECOND as i128) as u32
    }
    ...
    fn handle_write(&mut self, offset: u64, val: u32) -> Result<()> {
        match offset {
            ...
            RTCLR => {
                ...
                self.tick_offset = dbs_utils::time::seconds_to_nanoseconds(i64::from(val))
